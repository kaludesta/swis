...
class ProductivityPredictor:
    ...
    def train_model(self, data_path=None):
        """Train the productivity prediction model using the dataset"""
        try:
            if data_path is None:
                # Default relative path from this script's location
                base_dir = os.path.dirname(os.path.abspath(__file__))
                data_path = os.path.join(base_dir, 'Datasets', 'ultimate_student_productivity_dataset_5000.csv')
...
if __name__ == "__main__":
    # If run as a script, train the model
